...
This study uses `google/gemma-4-12B`, `Qwen/Qwen3.5-9B` and `google/gemma-4-E4B` on the supplied A100 PCIe 40 GB. The Qwen2.5-1.5B and 7B runs are supplementary controls in the separate `graph_size_pilot` implementation. See `experiment_status.json` and [W&B](https://wandb.ai/nileshsarkar-ai/saturatellm-feasibility) for completion status.
...
```bash
python pilot.py prepare --model google/gemma-4-12B --out prepared
...
`orchestrate.py --model MODEL_ID` runs this sequence with W&B, followed by separate default/matched profiler calls at batch 63. It expects the private credential file `/home/.saturatellm-auth/wandb.key`. Adapt that path for reproduction; do not commit credentials. The modern detached queue launch was:

```bash
jl run --on 500717 --json --yes -- bash -lc 'cd /home/SaturateLLM/model_suite && /home/SaturateLLM/.venv/bin/python queue_models.py'
```
...
The saved queue scripts contain run-specific dependency IDs: `r_739e084f` ran Gemma 4 12B then Qwen3.5-9B; `r_7a2c52f8` waited for that queue and ran Gemma 4 E4B. For a fresh study, run `orchestrate.py --model MODEL_ID` directly inside a detached job, or replace dependency IDs deliberately. `language_model_only=True` disables unused image/audio components. No quantization is used.

After all three folders are complete, run `summarize_suite.py FOLDER1 FOLDER2 FOLDER3 --out SUMMARY_DIR`, then `plot_suite.py SUMMARY_DIR/suite.json --out FIGURE_DIR`. These scripts refuse incomplete comparisons or mismatched source/data/runtime provenance. They are analysis tools, not additional GPU experiments.
