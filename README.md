...
- [Final presentation](Inference_Optimization_Final.pptx): the single current PPT, with the research question, background, published results, all distinct experimental graph panels and references.
...
- [Presentation source](outputs/presentation_source/README.md).

Original environment: Python 3.12.13, vLLM 0.28.0, PyTorch 2.13.0 and Transformers 5.16.1. Full package lists are saved with each run. A compatible NVIDIA GPU and valid W&B authentication are needed for reruns. Credentials and model-weight caches are not included.
