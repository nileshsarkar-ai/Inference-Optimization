...
root=Path('outputs');d=json.loads((root/'scheduling_results/analysis/validation.json').read_text())
metrics=['slo_goodput_rps','output_tokens_s','p99_ttft_s','p99_stream_gap_s','mean_gpu_busy_percent']
rows=[]
for p,label in [('default','Current vLLM'),('fixed32','Fixed limit 32'),('context_budget','Context budget')]:
 r=[x for x in d['observations'] if x['policy']==p]
 rows.append('| '+label+' | '+' | '.join(f'{statistics.mean(x[k] for x in r):.3f}' for k in metrics)+' |')
text='''# Inference Optimization measured results
...
Scheduling W&B runs:
'''+''.join(f'\n- [Independent repetition {i}]({u})' for i,u in enumerate(d['wandb_runs']))+'\n'
