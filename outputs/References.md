...
Checked 8 September 2026. The PPT includes clickable source footers and citations in speaker notes. Original paper excerpts remain separate from our own measured graphs. Cross-paper gains are not directly comparable.
...
4. **Agrawal et al. (2024). Taming Throughput-Latency Tradeoff in LLM Inference with Sarathi-Serve. OSDI 2024.** [USENIX PDF](https://www.usenix.org/system/files/osdi24-agrawal.pdf). Slide 4. Background on chunked prefill and batching.

5. **Aegaeon (2025). Effective GPU Pooling for Concurrent LLM Serving on the Market. SOSP 2025.** [Author PDF](https://ennanzhai.github.io/pub/sosp25-aegaeon.pdf). Slide 4. Token-level model autoscaling and GPU pooling. No Aegaeon numeric result is used in the 15-slide deck.

6. **Prism: Cost-Efficient Multi-LLM Serving via GPU Memory Ballooning (2026). OSDI 2026.** [USENIX technical-session listing](https://www.usenix.org/conference/osdi26/technical-sessions). Slide 4. This LLM system is distinct from the NSDI 2025 recommendation-model system above. No numeric result is used in the deck.
...
11. **NVIDIA NVML utilization definition.** [nvmlUtilization_t](https://docs.nvidia.com/deploy/nvml-api/structnvmlUtilization__t.html). Slide 2. Kernel-busy percentage is different from achieved SM efficiency or useful completed work.

## Models and data

12. Official model repositories: [Gemma 4 12B](https://huggingface.co/google/gemma-4-12B), [Gemma 4 E4B](https://huggingface.co/google/gemma-4-E4B), [Qwen3.5-9B](https://huggingface.co/Qwen/Qwen3.5-9B). Slides 8–10. The result folders record immutable revisions and exact package versions. E4B indicates effective active parameters; it should not be described as a four-billion-total-parameter model.
...
14. [SaturateLLM W&B project](https://wandb.ai/nileshsarkar-ai/saturatellm-feasibility). Slides 9–14. Capture-comparison JSONL records, raw output token IDs, runtime configuration and independent scheduling request/telemetry records are saved locally. The summary CSVs and full-resolution PNG/PDF graphs are supplied with the deck. These results are our measurements, not published-paper results.
