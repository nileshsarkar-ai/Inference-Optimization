...
group = datetime.datetime.now(datetime.timezone.utc).strftime('a100-pilot-%Y%m%dT%H%M%SZ')
...
subprocess.run([sys.executable, "pilot.py", "prepare", "--out", str(result / "prepared")], check=True)
...
print('Pilot complete:', result, flush=True)
